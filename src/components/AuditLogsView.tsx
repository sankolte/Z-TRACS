...
export const AuditLogsView: React.FC<AuditLogsViewProps> = ({
  ...
  const currentDistrict = currentUser?.district || 'Statewide';
  const isStateAdmin = currentRole === 'STATE_ADMIN' || currentRole === 'STATE_AUDITOR';
  const isDistrictScoped = !isStateAdmin && currentDistrict !== 'Statewide (All)' && currentDistrict !== 'Statewide';
  ...
  // Extract unique districts from existing logs for filter dropdown
  const availableDistricts = Array.from(
    new Set(logs.map(l => l.district).filter(Boolean))
  ).sort();
  ...
  // 1. Jurisdiction Authorization Filter (Statewide vs District-Locked)
  const authorizedLogs = logs.filter(log => {
    if (isDistrictScoped) {
      const targetDist = (currentDistrict || '').trim().toLowerCase();
      const logDist = (log.district || '').trim().toLowerCase();
      return logDist === targetDist || (log.user?.role || '').toLowerCase().includes(targetDist);
    }
    if (districtFilter !== 'ALL') {
      return (log.district || '').trim().toLowerCase() === districtFilter.trim().toLowerCase();
    }
    return true;
  });

  // KPI Counts based on authorized jurisdiction
  const totalLogs = authorizedLogs.length;
  const authLogsCount = authorizedLogs.filter(l => isAuthAction(l.action)).length;
  const cameraLogsCount = authorizedLogs.filter(l => isCameraAction(l.action)).length;
  const userAdminLogsCount = authorizedLogs.filter(l => isUserAdminAction(l.action)).length;
  const intelLogsCount = authorizedLogs.filter(l => isIntelligenceAction(l.action)).length;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className={`rounded-2xl text-white p-6 border shadow-2xl space-y-4 ${
        isDistrictScoped 
          ? 'bg-gradient-to-r from-[#1A120B] via-[#2A1E14] to-[#3B2A1C] border-amber-800/80' 
          : 'bg-gradient-to-r from-[#06162E] via-[#092248] to-[#0A2E63] border-blue-800'
      }`}>
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center space-x-3.5">
            <div className={`p-3.5 rounded-xl border shadow-inner ${
              isDistrictScoped ? 'bg-amber-600/30 border-amber-500/50' : 'bg-blue-600/30 border-blue-500/50'
            }`}>
              {isDistrictScoped ? <Lock className="w-7 h-7 text-amber-400" /> : <Shield className="w-7 h-7 text-amber-400" />}
            </div>
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h2 className="text-lg font-black text-white uppercase tracking-wider">
                  {isDistrictScoped ? `${currentDistrict} District Immutable Audit Ledger` : 'Statewide Immutable Audit Ledger'}
                </h2>
                {isDistrictScoped ? (
                  <span className="px-2.5 py-0.5 rounded text-[10px] font-mono font-bold bg-amber-500/20 text-amber-300 border border-amber-500/40">
                    🔒 LEVEL 4: DISTRICT-LOCKED ({currentDistrict.toUpperCase()})
                  </span>
                ) : (
                ...
                <span className="px-2.5 py-0.5 rounded text-[10px] font-mono font-bold bg-blue-500/20 text-blue-300 border border-blue-500/40">
                  ● SHA-256 MERKLE CHAIN SEALED
                </span>
              </div>
              <p className="text-xs text-blue-200 mt-1 max-w-3xl">
                {isDistrictScoped
                  ? `Cryptographic audit ledger locked to ${currentDistrict} jurisdiction. Statewide classified records and external district operation trails are redacted in compliance with Gujarat Police statutory data access governance.`
                  : 'Tamper-proof, cryptographic audit ledger tracking all operator logins, user creations/deletions, camera onboardings, camera deletions, and live stream telemetry across all 33 Gujarat districts.'}
              </p>
            </div>
          </div>
        ...
        {verifySuccess && (
          <div className="p-3.5 rounded-xl bg-emerald-950/80 border border-emerald-500 text-emerald-200 text-xs font-mono flex items-center space-x-2.5 animate-in zoom-in-95">
            ...
            <span>
              <strong>SHA-256 Merkle Chain Passed:</strong> All {authorizedLogs.length} audit entries verified against State Data Center root cryptographic signatures. 0% tampering detected.
            </span>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-4 bg-white border border-slate-200 rounded-xl shadow-xs space-y-1">
          <div className="flex items-center justify-between text-xs text-slate-500 font-bold uppercase tracking-wider">
            <span>{isDistrictScoped ? `${currentDistrict} Events` : 'Total Logged Events'}</span>
      ...
      <div className="p-4 rounded-xl bg-white border border-slate-200 space-y-3 shadow-xs">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
          ...
          <div className="flex items-center space-x-2">
            {/* District Filter: Dropdown for State Admin, Locked Badge for District Admin */}
            {isDistrictScoped ? (
              <div className="px-3 py-2 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-900 font-bold flex items-center space-x-1.5 shadow-xs">
                <Lock className="w-3.5 h-3.5 text-amber-600" />
                <span>Jurisdiction: {currentDistrict}</span>
              </div>
            ) : (
            ...
            {(search || actionFilter !== 'All' || categoryTab !== 'ALL' || districtFilter !== 'ALL') && (
              <button
                onClick={() => { setSearch(''); setActionFilter('All'); setCategoryTab('ALL'); setDistrictFilter('ALL'); }}
